...
def ilsvrc2010(input_directory, save_path,
    ...
    test_groundtruth_path = os.path.join(input_directory, TEST_GROUNDTRUTH)
    ...
    with h5py.File(os.path.join(save_path, output_filename), 'w-') as f:
        ...
        process_train_set(f, train, patch, 50,  # n_train,
                          wnid_map, shuffle_seed)
        process_other_set(f, 'valid', valid, patch, valid_groundtruth[:50],
                          n_train)
        process_other_set(f, 'test', test, patch, test_groundtruth[:50],
                          n_train + n_valid)
...
def prepare_hdf5_file(f, n_train, n_valid, n_test):
    ...
    f.create_dataset('encoded_images', shape=(n_total,),
                     dtype=vlen_dtype)
...
def image_consumer(socket, hdf5_file, num_expected, shuffle_seed=None,
    ...
    for num in index_gen:
        ...
        print(image_filename, offset + num)
